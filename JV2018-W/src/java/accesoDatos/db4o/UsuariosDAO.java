...
public class UsuariosDAO  implements OperacionesDAO {
	...
	private UsuariosDAO() {
		db = Conexion.getDB();
		db.store(new HashMap <String,String>());
	...
	private void registrarEquivalenciaId(Usuario usuario) {
		...
		HashMap<String,String> mapaEquivalencias = obtenerMapaEquivalencias();
	 ...
	 */
	private HashMap <String, String> obtenerMapaEquivalencias() {
		//Obtiene mapa de equivalencias de id de acceso
		Query consulta = db.query();
		consulta.constrain(HashMap.class);
		ObjectSet <HashMap <String,String>> result = consulta.execute();
		return result.get(0);	
	}
